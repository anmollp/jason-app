    ...
    isDiff
      ...
      : isPointer
        ? isOverPointerPayloadLimit
          ...
          ? "Calling POST /pointer on the backend."
          : pointerState === "error"
            ? "Update the JSON Pointer path, then find again."
            : pointerState === "success"
              ? "The matching JSON value is highlighted in the source editor."
              : "Select a JSON value or search for a JSON Pointer path."
        : isOverPayloadLimit
